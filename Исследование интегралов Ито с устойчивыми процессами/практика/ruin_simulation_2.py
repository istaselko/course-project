...
u = 0.5  # уменьшил начальный капитал
c = 1.0  # Годовая страховая премия (доход компании)
X_port = 25.0  # Увеличил объем портфеля в 25 раз (беру больше рисков)
...
# Центрирую шум, чтобы математическое ожидание скачков было строго нулем.
# Это нужно для того, чтобы плавный рост задавался исключительно премией 'c'.
xi_matrix = xi_matrix - np.mean(xi_matrix)
